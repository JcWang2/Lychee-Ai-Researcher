# Upload v2.5.6 (pushed to Lychee-Ai-Researcher): row-cap label
# alignment + string-target lookup renderer + compound-id joins;
# SHA read from the sidecar; self-locating.
...
$SshPort = 9000
$SshUser = "xzr"
$PackageName = "ai_scientist_execution_layer_v2_20260808_v256.tar.gz"
...
$PackageDir = Join-Path $PSScriptRoot "ai_scientist_execution_layer_v2_20260808_v256"
...
try {
    scp -P $SshPort $Package.Name (Split-Path -Leaf $Sidecar) `
        "run_v2_a100_3tasks_v23.sh" "run_v2_a100_lite_v256.sh" `
        "monitor_v2_v23.sh" "monitor_v2_v256_live.sh" `
...
ssh -p $SshPort $Remote "cd /mnt/data/stage42_delivery/incoming && echo '$ExpectedSha256  $PackageName' | sha256sum -c - && bash -n run_v2_a100_3tasks_v23.sh && bash -n run_v2_a100_lite_v256.sh && bash -n monitor_v2_v23.sh && bash -n monitor_v2_v256_live.sh"
...
Write-Host "Next (on the server) install v2.5.6 into the deploy tree:"
Write-Host "  cd /mnt/data/stage42_delivery/incoming"
Write-Host "  tar -xzf ai_scientist_execution_layer_v2_20260808_v256.tar.gz"
Write-Host "  cd ai_scientist_execution_layer_v2_20260808_v256 && sha256sum -c MANIFEST.sha256"
...
Write-Host '  # expect V2_PACKAGE_MANIFEST=PASS / V2_PYCOMPILE=PASS / V2_OFFLINE_TESTS=PASS (incl. test_v2_256) / V2_INSTALL_VERIFY=PASS'
Write-Host "  # Then retest (TASK_LIST overridable, default = 3 image tasks):"
Write-Host "  #   nohup bash run_v2_a100_lite_v256.sh > run_v2_lite_v256_outer.log 2>&1 &"
Write-Host "  #   bash monitor_v2_v256_live.sh"
